...
from backend.app.ml.xgboost_return_regressor_evaluator import (
    XGBoostReturnRegressorEvaluator,
)
...
def test_xgboost_return_regressor_evaluator_calculates_metrics_by_horizon():
    ...
    metrics_by_horizon = (
        XGBoostReturnRegressorEvaluator.evaluate(
            model=model,
...
def test_xgboost_return_regressor_evaluator_uses_sample_features():
    training_dataset = MLDataset(
        samples=[
            create_sample(
                feature_timestamp=1_800_000_000_000,
...
def test_xgboost_return_regressor_evaluator_does_not_refit_model():
    training_dataset = MLDataset(
        samples=[
            create_sample(
                feature_timestamp=1_800_000_000_000,
...
def test_xgboost_return_regressor_evaluator_rejects_invalid_model(
    model,
):
    dataset = MLDataset(
        samples=[
    ...
    with pytest.raises(
        TypeError,
        match="model must be an XGBoostReturnRegressor",
    ):
        XGBoostReturnRegressorEvaluator.evaluate(
            model=model,
            dataset=dataset,
...
def test_xgboost_return_regressor_evaluator_rejects_invalid_dataset(
    dataset,
):
    model = XGBoostReturnRegressor()
...
def test_xgboost_return_regressor_evaluator_rejects_unfitted_model():
    dataset = MLDataset(
        samples=[
            create_sample(
                feature_timestamp=1_800_000_000_000,
...
def test_xgboost_return_regressor_evaluator_rejects_unseen_horizon():
    training_dataset = MLDataset(
        samples=[
            create_sample(
                feature_timestamp=1_800_000_000_000,
